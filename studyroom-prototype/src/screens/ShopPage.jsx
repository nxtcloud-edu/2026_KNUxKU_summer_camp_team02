...
function CharCard({ char, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-[140px] sm:w-[180px] shrink-0 rounded-lg border border-hairline bg-[var(--hover-bg)] p-4 text-left transition-colors duration-200 hover:bg-white hover:shadow-soft"
    >
      {/* 이미지 플레이스홀더 (회색 네모) */}
  ...
  return (
    <div className="relative min-h-full overflow-hidden bg-warm">
      ...
      {showBasicPopup && (
        ...
        >
          <div
            className="relative w-full max-w-[820px] rounded-lg border border-hairline bg-surface p-8 shadow-soft"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              type="button"
