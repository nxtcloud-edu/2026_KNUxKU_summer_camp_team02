...
export function Button({ variant = 'secondary', shape = 'pill', children, className = '', ...rest }) {
  ...
  return (
    <button className={`${base} ${shapes[shape] || shapes.pill} ${styles[variant] || styles.secondary} ${className}`} {...rest}>
      {children}
    </button>
  )
}
...
export function Dialog({ open, onClose, title, children, footer, width = 1100, height = 760, minWidth = 900, plain = false, labelledBy }) {
