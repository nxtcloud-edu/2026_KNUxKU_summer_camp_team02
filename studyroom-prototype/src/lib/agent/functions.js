...
/**
 * 값싼 모델에 맡겨도 되는 **일상 대화**인가.
 *
 * ⚠️ 이건 **내려보내는** 목록이다. 자료 첨부에 쓰던 낱말 목록과 방향이 반대라는 걸
 *    분명히 해 둔다. 거기서는 목록을 **맞혀야** 좋은 동작(자료 첨부)이 나왔고,
 *    그래서 못 맞힐 때마다 답이 틀렸다 — 두 번 샜고 결국 목록을 지웠다.
 *    여기서는 목록에 **안 걸리면** 좋은 동작(상위 모델)이 그대로 유지된다.
 *    못 맞히면 돈이 조금 더 들 뿐, 답이 나빠지지 않는다. 그래서 목록을 써도 된다.
 *
 * 그러니 넓게 잡지 않는다. 애매하면 걸리지 않게 두는 편이 맞다.
 *
 * ⚠️ 한국어에는 낱말 경계(\b)가 없다. `^그래` 는 "그래프 탐색"을, `^응` 은
 *    "응용 계층"을, `^어` 는 "어텐션"을 삼킨다. 뒤에 한글이 이어지면 걸러 낸다.
 */
/**
 * 짧아서 다른 말에 삼켜지는 것들. 뒤에 한글이 이어지면 제외한다.
 * `응`→응용 계층, `어`→어텐션, `와`→와이파이, `네`→네트워크, `그래`→그래프.
 */
const GREET_SHORT_RE =
  /^(?:응|어|웅|넵|넹|네|굿|헐|와|짱|대박|그래|ㅇㅇ|ㄴㄴ|ㅇㅋ|ㅎㅇ|ok|ㅋㅋ|ㅎㅎ)(?![가-힣])/i

/**
 * 세 자 이상이라 다른 말과 겹칠 일이 없는 것들.
 * 여기엔 한글 금지 조건을 걸지 않는다 — 걸면 `알겠`이 "알겠어"를 놓친다(어미 활용).
 */
const GREET_LONG_RE =
  /^(?:안녕|하이|헬로|반가|잘가|바이|잘자|굿나잇|고마워|고맙|감사|미안|죄송|오케이|알겠|알았|맞아|아니)/i

/** 몸 상태·잡담. 학습 질문과 겹치지 않는 것만 */
const MOOD_RE =
  /배고[프파]|졸[리려]|피곤|힘들다|심심|지루|덥다|춥다|날씨|점심|저녁|밥\s*먹|커피|화장실|기지개|스트레칭/

/**
 * **묻고 있으면 일상 대화가 아니다.** 인사로 시작해도 마찬가지다.
 * "응 그럼 캐시는 어떻게 동작해" 는 `응` 으로 시작하지만 명백히 학습 질문이다.
 * 이 목록은 **내려보내기를 막는** 쪽으로만 작동한다 — 못 맞혀도 답이 나빠지지 않는다.
 */
const ASK_RE =
  /뭐야|뭔데|뭐임|무엇|설명|알려|가르쳐|어떻게|어째서|왜\s|차이|원리|방법|정의|구현|동작|이유|예시|비교|알고리즘|프로토콜|\?/

/**
 * @param {string} text 사용자가 한 말
 * @returns {boolean} 일상 대화로 보고 값싼 모델에 맡길 것인가
 */
export function isSmallTalk(text) {
  const t = String(text || '').trim()
  // 길면 뭔가를 묻는 것이다. 인사에 스물넉 자를 쓰지 않는다
  if (!t || t.length > 24) return false
  if (ASK_RE.test(t)) return false
  return GREET_SHORT_RE.test(t) || GREET_LONG_RE.test(t) || MOOD_RE.test(t)
}
