 ...
 *
 * (예전 주석에 "high 는 쓰지 않는다"고 적혀 있었다. 그때는 예산이 2,000이라 잘렸던 것이고,
 *  지금 F1·F6 은 high 로 돈다. 몫을 키운 뒤로는 잘리지 않는다.)
 */
/**
 * 사고에 쓸 몫. 답변 몫과 **별도로** 더해진다.
 *
 * ⚠️ 이건 "이만큼 생각해라"가 아니라 **"이만큼까지 써도 된다"** 는 상한이다.
 *    안 쓰면 청구되지 않는다. 그러니 넉넉히 잡는 쪽의 손해는 없고,
 *    빠듯하게 잡으면 사고하다 답을 못 쓰고 잘린다. 한쪽만 위험한 저울이다.
 *
 * 그래서 값을 키우는 데에 지연 대가도 없다 — 지연은 모델이 **실제로 쓴** 사고량을
 * 따라가지, 상한을 따라가지 않는다. 시연 중에 잘리는 쪽이 훨씬 비싼 사고다.
 *
 * 상한을 아예 없앨 수는 없다. Gemini 는 maxOutputTokens 를 요구하고,
 * 이 값은 거기에 더해져 나간다. "무제한"에 가장 가까운 선택은
 * 모델이 한 번에 낼 수 있는 양보다 크게 잡아 두는 것이다.
 */
export const THINK_ALLOWANCE = { low: 0, medium: 4000, high: 10_000 }
